const STOCK_DATA = {
  "lastUpdated": "2026-05-28T22:00:00Z",
  "trackedStocks": [
    {"name": "立昂微", "code": "sh605358", "sector": "半导体"},
    {"name": "格力电器", "code": "sz000651", "sector": "家电"},
    {"name": "药明康德", "code": "sh603259", "sector": "医药"},
    {"name": "巨星科技", "code": "sz002444", "sector": "出海"},
    {"name": "西安奕材", "code": "sh688783", "sector": "半导体"},
    {"name": "粤电力A", "code": "sz000539", "sector": "电力"},
    {"name": "众源新材", "code": "sh603527", "sector": "材料"},
    {"name": "宁德时代", "code": "sz300750", "sector": "新能源"},
    {"name": "中芯国际", "code": "sh688981", "sector": "半导体"},
    {"name": "贵州茅台", "code": "sh600519", "sector": "白酒"}
  ],
  "currentWeek": {
    "weekLabel": "2026年第22周 (5/25 - 5/29)",
    "weekId": "2026-W22",
    "topPick": {
      "name": "立昂微",
      "code": "605358",
      "sector": "半导体硅片",
      "rating": "强烈推荐",
      "price": 59.09,
      "targetPrice": 77.00,
      "upside": "30%",
      "marketCap": "397亿",
      "pe": "扭亏期",
      "pb": 5.68,
      "dividendYield": "—",
      "riskLevel": "中等",
      "horizon": "中长期 (1-2年)",
      "entryZone": "53-55元 (10日均线附近)",
      "stopLoss": "42元 (60日均线失守)",
      "thesis": "12英寸半导体硅片国产化龙头，2026Q1扭亏为盈走过盈利拐点。台积电2026年资本开支560亿美元，AI服务器对硅片需求是传统服务器的3.8倍。三大增长引擎：12英寸硅片收入+88%、化合物半导体+92%、VCSEL+561%。中信证券目标价77元。",
      "catalysts": [
        "Q2产能利用率继续提升",
        "台积电/中芯国际扩产拉动硅片需求",
        "国产替代政策持续推进"
      ],
      "risks": [
        "毛利率爬坡速度不及预期",
        "高负债率(110%)带来利息压力",
        "在建工程转固后折旧增加"
      ]
    },
    "secondaryPicks": [
      {
        "name": "格力电器",
        "code": "000651",
        "sector": "家电",
        "rating": "买入",
        "badge": "buy",
        "price": 40.20,
        "targetPrice": 52.00,
        "upside": "29%",
        "pe": 8.2,
        "dividendYield": "7.72%",
        "riskLevel": "低",
        "horizon": "长期 (3-5年)",
        "entryZone": "38-41元分批建仓",
        "stopLoss": "34元",
        "thesis": "空调龙头+高股息标杆。PE仅8倍处历史底部，股息率7.72%为A股最高一档。夏季高温+以旧换新补贴驱动销量确定性增长。账上现金充沛，分红可持续。"
      },
      {
        "name": "药明康德",
        "code": "603259",
        "sector": "医药CXO",
        "rating": "买入",
        "badge": "buy",
        "price": 61.50,
        "targetPrice": 85.00,
        "upside": "38%",
        "pe": 18.3,
        "dividendYield": "1.2%",
        "riskLevel": "中等",
        "horizon": "中长期 (1-3年)",
        "entryZone": "55-62元左侧布局",
        "stopLoss": "48元",
        "thesis": "全球CXO龙头，PE 18倍处于近5年最低分位。海外药企研发外包需求回暖，Q1新签订单出现拐点。地缘政治风险被市场过度定价。左侧布局困境反转。"
      },
      {
        "name": "巨星科技",
        "code": "002444",
        "sector": "出海制造",
        "rating": "关注",
        "badge": "watch",
        "price": 31.80,
        "targetPrice": 42.00,
        "upside": "32%",
        "pe": 16.1,
        "dividendYield": "2.5%",
        "riskLevel": "中等",
        "horizon": "中长期 (2-3年)",
        "entryZone": "28-32元区间",
        "stopLoss": "25元",
        "thesis": "全球手工具龙头，境外收入占比95%，A股最纯正出海标的。美国地产周期复苏+DIY文化驱动需求。PE仅16倍明显低估。入选证券时报2026年26只潜力股。"
      }
    ]
  },
  "history": [
    {
      "weekId": "2026-W21",
      "weekLabel": "2026年第21周 (5/18 - 5/22)",
      "topPick": "立昂微",
      "picks": [
        {"name": "立昂微", "code": "605358", "sector": "半导体", "entryPrice": 55.78, "currentPrice": 59.09, "perf": "+5.9%"},
        {"name": "格力电器", "code": "000651", "sector": "家电", "entryPrice": 39.50, "currentPrice": 40.20, "perf": "+1.8%"},
        {"name": "巨星科技", "code": "002444", "sector": "出海", "entryPrice": 30.50, "currentPrice": 31.80, "perf": "+4.3%"}
      ]
    },
    {
      "weekId": "2026-W20",
      "weekLabel": "2026年第20周 (5/11 - 5/15)",
      "topPick": "宁德时代",
      "picks": [
        {"name": "宁德时代", "code": "300750", "sector": "新能源", "entryPrice": 245.00, "currentPrice": 258.50, "perf": "+5.5%"},
        {"name": "阳光电源", "code": "300274", "sector": "新能源", "entryPrice": 118.00, "currentPrice": 125.30, "perf": "+6.2%"},
        {"name": "中芯国际", "code": "688981", "sector": "半导体", "entryPrice": 88.50, "currentPrice": 92.10, "perf": "+4.1%"}
      ]
    },
    {
      "weekId": "2026-W19",
      "weekLabel": "2026年第19周 (5/4 - 5/8)",
      "topPick": "贵州茅台",
      "picks": [
        {"name": "贵州茅台", "code": "600519", "sector": "白酒", "entryPrice": 1680.00, "currentPrice": 1720.00, "perf": "+2.4%"},
        {"name": "恒瑞医药", "code": "600276", "sector": "医药", "entryPrice": 52.30, "currentPrice": 54.80, "perf": "+4.8%"},
        {"name": "立昂微", "code": "605358", "sector": "半导体", "entryPrice": 48.50, "currentPrice": 55.78, "perf": "+15.0%"}
      ]
    },
    {
      "weekId": "2026-W18",
      "weekLabel": "2026年第18周 (4/27 - 5/1)",
      "topPick": "美的集团",
      "picks": [
        {"name": "美的集团", "code": "000333", "sector": "家电", "entryPrice": 72.00, "currentPrice": 74.50, "perf": "+3.5%"},
        {"name": "长江电力", "code": "600900", "sector": "电力", "entryPrice": 28.50, "currentPrice": 29.10, "perf": "+2.1%"},
        {"name": "金山办公", "code": "688111", "sector": "软件", "entryPrice": 385.00, "currentPrice": 372.00, "perf": "-3.4%"}
      ]
    }
  ]
};
